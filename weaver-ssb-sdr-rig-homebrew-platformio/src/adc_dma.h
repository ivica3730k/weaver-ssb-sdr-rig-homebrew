...
void sample_iq_adc(uint16_t& result1, uint16_t& result2) {
    while (!adc_conversion_complete) {
        // Waiting for ADC conversion to complete
        // Add your code to handle the waiting condition
    }

    adc_conversion_complete = false;

    result1 = aADCxConvertedValues[0];
    result2 = aADCxConvertedValues[1];
}